...
export default function BlessingsPage() {
  ...
  const [activeTab, setActiveTab] = useState<"sanctuary" | "maker" | "wallpapers" | "saved">(() => {
    if (tabParam === "sanctuary" || tabParam === "maker" || tabParam === "wallpapers" || tabParam === "saved") {
      return tabParam;
    }
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#180a06] via-[#0d0502] to-[#120603] text-amber-100 flex flex-col font-serif select-none relative overflow-x-hidden">
      ...
      <div className="w-full max-w-md mx-auto px-4 mt-4 select-none">
        <nav className="bg-[#120704]/70 p-1.5 rounded-full border border-amber-950/40 flex items-center shadow-lg">
          ...
          >
            <span>🏰</span>
            <span>{isHi ? "मंदिर गर्भगृह" : "Sanctuary"}</span>
          </button>
      ...
      <main className="flex-1 w-full max-w-md mx-auto px-4 py-5 pb-24 relative z-10 flex flex-col items-center">
        ...
        {/* ========================================================= */}
        {/* SECTION 1: TEMPLE SANCTUARY (DAILY DARSHAN & PUJA SEVA)   */}
        {/* ========================================================= */}
        {activeTab === "sanctuary" && (
          <div className="w-full space-y-5 flex flex-col items-center animate-fade-in">
            ...
            <div className="text-center px-2">
              <p className="text-xs text-amber-300/80 font-sans leading-relaxed">
                {isHi 
                  ? "घंटी बजाएं, दीप जलाएं, पुष्प अर्पण कर आज की साधना आरंभ करें"
                  : "Ring the temple bell, light a diya, offer flowers, and complete today's Sadhana"}
              </p>
            </div>

            {/* MAIN DARSHAN TEMPLE CARD */}
            <div className="w-full bg-[#1b0d07]/60 border border-amber-500/15 rounded-3xl p-5 shadow-2xl relative overflow-hidden flex flex-col items-center text-center">
              
              {/* Saffron Temple Arch Background Overlay */}
              ...
              {/* Day & Date banner */}
              <div className="flex items-center justify-between w-full mb-3 text-[10px] tracking-widest text-amber-500 font-sans uppercase font-black">
                <span>{new Date().toLocaleDateString(isHi ? "hi-IN" : "en-US", { weekday: "long" })}</span>
                <span>{new Date().toLocaleDateString(isHi ? "hi-IN" : "en-US", { month: "short", day: "numeric", year: "numeric" })}</span>
              </div>

              {/* High-Fi Deity Image Visual with Gold Frame */}
              <div className="w-full aspect-[4/5] rounded-2xl overflow-hidden border border-amber-500/20 shadow-2xl relative select-none bg-black/40">
                <img 
                  src={todayDarshan.imageUrl} 
                  alt={todayDarshan.deity} 
                  className="w-full h-full object-cover object-center pointer-events-none transition-transform duration-700 hover:scale-105" 
                />
                
                {/* Vignette bottom mask */}
                <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-transparent to-transparent flex flex-col justify-end p-5 text-left pointer-events-none" />
                
                {/* Lit Diya animation overlay */}
                {isDiyaLit && (
                  <motion.div 
                    initial={{ opacity: 0, scale: 0.8 }}
                    animate={{ opacity: 1, scale: 1 }}
                    className="absolute bottom-4 left-1/2 -translate-x-1/2 pointer-events-none z-10 flex flex-col items-center"
                  >
                    <img 
                      src={litDiyaImg} 
                      alt="Lit Diya" 
                      className="w-24 h-auto select-none pointer-events-none filter drop-shadow-[0_4px_10px_rgba(251,191,36,0.6)]" 
                    />
                    <div className="w-4 h-6 bg-orange-500 rounded-full animate-flame absolute -top-1.5 left-[46px]" />
                  </motion.div>
                )}

                {/* Ambient floating flower petals inside deity frame */}
                <div className="absolute bottom-4 left-4 right-4 text-left pointer-events-none z-10">
                  <h3 className="font-serif text-xl font-black text-white leading-tight drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">
                    {isHi ? todayDarshan.deityHindi : todayDarshan.deity}
                  </h3>
                  <p className="font-sans text-[10px] text-amber-400 font-bold uppercase tracking-widest leading-none mt-1.5 drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)]">
                    📍 {isHi ? todayDarshan.templeNameHindi : todayDarshan.templeName}
                  </p>
                </div>
              </div>

              {/* Devotional Prasad Verse of the Day */}
              <div className="my-4.5 px-1.5 select-text relative">
                <p className="font-serif text-sm md:text-base leading-relaxed text-amber-100/90 italic font-semibold">
                  "{isHi ? todayDarshan.quoteHindi : todayDarshan.quote}"
                </p>
              </div>

              {/* ─── VIRTUAL PUJA SEVA ACTIONS BAR ───────────────────────── */}
              <div className="w-full border-t border-amber-950/20 pt-4 mt-2 grid grid-cols-4 gap-2 font-sans select-none">
                
                {/* Ring Bell */}
                <button
                  onClick={triggerBell}
                  className="flex flex-col items-center gap-1 py-2.5 rounded-xl border border-amber-500/10 hover:border-amber-500/20 bg-black/30 text-amber-300 transition-all active:scale-95 focus:outline-none"
                >
                  <motion.div
                  ...
                  <span className="text-[9px] font-bold uppercase tracking-wider">{isHi ? "घंटी बजाएं" : "Ring Bell"}</span>
                </button>
                ...
                >
                  ...
                  <span className="text-[9px] font-bold uppercase tracking-wider">{isHi ? "पुष्प अर्पण" : "Offer Flowers"}</span>
                </button>

                {/* Light Diya */}
                <button
                  onClick={toggleDiya}
                  className={`flex flex-col items-center gap-1 py-2.5 rounded-xl border transition-all active:scale-95 focus:outline-none ${
                    isDiyaLit 
                      ? "border-amber-500 bg-amber-500/10 text-amber-400 font-bold" 
                      : "border-amber-500/10 bg-black/30 text-amber-300"
                  }`}
                >
                  <Flame className={`w-5 h-5 ${isDiyaLit ? "text-orange-500 animate-pulse fill-current" : "text-amber-500/60"}`} />
                  <span className="text-[9px] font-bold uppercase tracking-wider">{isHi ? "दीप जलाएं" : "Light Diya"}</span>
                </button>
                ...
                <button
                  ...
                  className={`flex flex-col items-center gap-1 py-2.5 rounded-xl border transition-all active:scale-95 focus:outline-none ${
                    isAudioPlaying 
                      ? "border-amber-500 bg-amber-500/10 text-amber-400 font-bold animate-[pulse_2s_infinite]" 
                      : "border-amber-500/10 bg-black/30 text-amber-300"
                  }`}
                >
                  {isAudioPlaying ? (
                    ...
                    <VolumeX className="w-5 h-5 text-amber-400/50" />
                  )}
                  <span className="text-[9px] font-bold uppercase tracking-wider">{isHi ? "मंत्र ध्वनि" : "Play Chants"}</span>
                </button>
              </div>
            </div>

            {/* ─── WEEKLY NITYA SADHANA PATHWAY (STREAK MAP) ─────────────── */}
            <div className="w-full bg-[#1b0d07]/40 border border-amber-950/30 rounded-3xl p-5 shadow-2xl flex flex-col gap-3">
              <div className="flex justify-between items-center border-b border-amber-950/20 pb-2.5 select-none">
                <h3 className="text-xs font-sans font-black text-amber-400 uppercase tracking-widest flex items-center gap-1.5">
                  <Award className="w-4 h-4 text-amber-400" />
                  {isHi ? "दैनिक साधना पथ (Nitya Sadhana Path)" : "Daily Sadhana Path"}
                </h3>
                <span className="text-[10px] font-sans font-bold text-amber-200/50">
                  {completedDates.includes(todayDateString) 
                    ? (isHi ? "आज का दर्शन पूर्ण! ✨" : "Today's Darshan Complete! ✨")
                    : (isHi ? "दर्शन व सेवा शेष" : "Puja / Offerings Pending")}
                </span>
              </div>

              {/* 7 Days Row */}
              <div className="grid grid-cols-7 gap-1 font-sans select-none">
                {WEEKDAYS.map((day) => {
                  const dayDate = weekDates[day.dayNum];
                  const isDone = completedDates.includes(dayDate);
                  const isToday = dayDate === todayDateString;
                  const isFuture = new Date(dayDate) > new Date();

                  return (
                    <div 
                      key={day.dayNum} 
                      className={`flex flex-col items-center gap-1.5 p-1.5 rounded-xl border text-center transition-all ${
                        isDone 
                          ? "bg-amber-500/10 border-amber-500/40 text-amber-300 shadow-md"
                          : isToday
                          ? "border-amber-500/30 bg-black/20 text-amber-400 animate-pulse"
                          : "border-amber-950/20 bg-black/40 text-amber-200/20"
                      }`}
                    >
                      <span className="text-[9px] font-bold uppercase tracking-wide">
                        {isHi ? day.labelHi : day.label}
                      </span>
                      
                      <div className="relative flex items-center justify-center">
                        {isDone ? (
                          <div className="relative">
                            <img src={litDiyaImg} alt="lit" className="w-6 h-auto filter drop-shadow-[0_1px_3px_rgba(251,191,36,0.6)]" />
                            <div className="w-1 h-2 bg-orange-500 rounded-full animate-flame absolute -top-0.5 left-[11px]" />
                          </div>
                        ) : isToday ? (
                          <div className="w-5 h-5 rounded-full border border-dashed border-amber-500/40 flex items-center justify-center bg-amber-500/5">
                            <span className="text-[8px] font-black text-amber-500/70">🪔</span>
                          </div>
                        ) : isFuture ? (
                          <Lock className="w-3.5 h-3.5 text-amber-200/5" />
                        ) : (
                          <div className="w-4 h-4 rounded-full bg-amber-950/20 border border-amber-950/30 flex items-center justify-center">
                            <div className="w-1.5 h-1.5 rounded-full bg-amber-200/5" />
                          </div>
                        )}
                      </div>
                      <span className="text-[7px] tracking-wide font-black truncate max-w-full uppercase text-amber-200/40">
                        {day.deity}
                      </span>
                    </div>
                  );
            ...
            {/* CALL TO ACTION: PERSONALIZATION TAB REROUTING */}
            <button
              onClick={() => setActiveTab("maker")}
              className="w-full py-4 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-black font-sans font-black text-xs uppercase tracking-widest rounded-2xl transition-all active:scale-[0.98] flex items-center justify-center gap-2 shadow-lg shadow-amber-500/20"
            >
              <Sparkles className="w-4 h-4 text-black fill-current animate-spin" />
              <span>{isHi ? "आज का कस्टमाइज्ड आशीर्वाद साझा करें" : "Share Today's Customized Blessing"}</span>
            </button>
