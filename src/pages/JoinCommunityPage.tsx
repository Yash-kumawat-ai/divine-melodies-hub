...
import { 
  ChevronLeft, Plus, Users, MessageSquare, Search, Copy, Globe, Info, Clock, Check, Trash2, Calendar, MapPin, ExternalLink, Sparkles, AlertCircle, Play, Heart, ThumbsUp, Send, User, ChevronRight
} from "lucide-react";
    ...
    <div className="min-h-screen bg-[#FAF6EE] dark:bg-[#0c0a08] text-stone-900 dark:text-stone-100 pb-20">
      ...
      <Dialog open={createGroupOpen} onOpenChange={setCreateGroupOpen}>
        <DialogContent className="max-w-md bg-[#FAF6EE] dark:bg-[#0f0d0a] border-orange-500/20 text-stone-950 dark:text-stone-50 rounded-3xl p-6 max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="font-display font-extrabold text-lg text-orange-950 dark:text-amber-100 text-center">
              {isHi ? "नया नाम संघ बनाएं" : "Create Naam Sangh Group"}
            </DialogTitle>
            <DialogDescription className="text-center text-xs text-stone-500 mt-1">
              {isHi ? "अपने भक्तों के समुदाय का निर्माण करें।" : "Establish a gathering space for devotees."}
            </DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={handleCreateGroup} className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-stone-600 dark:text-stone-300">
                {isHi ? "नाम संघ का नाम" : "Group Name"}
              </label>
              <Input 
                type="text"
                placeholder={isHi ? "जैसे: हनुमान सत्संग मंडल" : "e.g., Shri Ram Bhajan Mandali"}
                value={groupName}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-stone-600 dark:text-stone-300">
                {isHi ? "विवरण (एक पंक्ति)" : "Description (One Line)"}
              </label>
              <Input 
                type="text"
                placeholder={isHi ? "जैसे: प्रतिदिन हनुमान चालीसा और भजन जप" : "e.g., Group for daily Kirtan practice"}
                value={groupDesc}
            ...
            {/* Choose Ishta Dev deity avatars grid */}
            <div className="space-y-2">
              <label className="text-xs font-bold text-stone-600 dark:text-stone-300 block">
                {isHi ? "इष्ट देव चुनें" : "Select Ishta Dev"}
              </label>
              <div className="grid grid-cols-4 gap-2">
                {DEITIES.map(d => (
                  <button
                    ...
                    className={`relative rounded-xl overflow-hidden aspect-square border flex flex-col items-center justify-center p-1 transition-all ${
                      groupDeity === d.id 
                        ? "border-orange-500 scale-[1.03] shadow-xs" 
                        : "border-stone-200 dark:border-stone-800 hover:border-orange-500/35"
                    }`}
                  >
                    <img src={d.src} alt={d.name} className="w-full h-full object-cover rounded-lg" />
                    <span className="text-[7.5px] font-bold text-center mt-1 truncate max-w-full text-stone-950 dark:text-stone-50">
                      {d.name.split(" ")[0]}
                    </span>
                    {groupDeity === d.id && (
                      <div className="absolute top-1 right-1 w-4 h-4 bg-orange-500 text-white rounded-full flex items-center justify-center text-[9px] font-bold">
                        ✓
                      </div>
                    )}
                  </button>
            ...
            <div className="flex gap-3 pt-3">
              ...
              <Button
                ...
                className="flex-1 bg-orange-500 hover:bg-orange-600 text-white font-bold rounded-xl"
              >
                {creatingGroup ? (isHi ? "बन रहा है..." : "Creating...") : (isHi ? "समूह बनाएं" : "Create Group")}
              </Button>
            </div>
          </form>
        </DialogContent>
