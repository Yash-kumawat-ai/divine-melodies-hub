...
import { Sparkles, Clock, ChevronRight, ArrowRight, User } from 'lucide-react';
...
const KundliQuickSummaryInner: React.FC<KundliQuickSummaryProps> = ({
  ...
  return (
    <div id="summary" className="space-y-3.5 w-full">
      ...
      {/* ── ABOUT YOU / ARCHETYPE SUMMARY CARD ── */}
      {(() => {
        try {
          const intel = generateAboutYouIntelligence(kundli);
          return (
            <button
              type="button"
              onClick={() => onSelectTab?.('about')}
              className="w-full text-left flex items-center justify-between gap-3 p-3 sm:p-3.5 rounded-2xl bg-brand-gold/10 hover:bg-brand-gold/15 border border-brand-gold/30 hover:border-brand-gold/50 shadow-2xs group transition-all cursor-pointer"
            >
              <div className="flex items-center gap-2.5 min-w-0">
                <div className="h-9 w-9 rounded-xl bg-brand-primary/10 text-brand-primary dark:text-brand-gold flex items-center justify-center shrink-0">
                  <User className="h-4 w-4" />
                </div>
                <div className="min-w-0">
                  <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
                    {isHi ? 'व्यक्तित्व प्रारूप' : 'Your Vedic Archetype'}
                  </p>
              ...
              <div className="shrink-0 flex items-center gap-1 text-xs font-semibold text-brand-primary dark:text-brand-gold group-hover:translate-x-0.5 transition-transform">
