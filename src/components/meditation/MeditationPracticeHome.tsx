...
const PinkLotusSvg = ({ className = "w-5 h-5", fill = "#ec4899", opacity = 0.95 }: { className?: string; fill?: string; opacity?: number }) => (
...
export default function MeditationPracticeHome({ onSelectPractice, onQuickStart }: MeditationPracticeHomeProps) {
  ...
  const formatMeditationTime = (totalMins: number) => {
    ...
    if (hrs === 0) {
      return isHi ? `${mins} मिनट` : `${mins} min`;
    }
    if (mins === 0) {
      return isHi ? `${hrs} घंटे` : `${hrs} h`;
    }
