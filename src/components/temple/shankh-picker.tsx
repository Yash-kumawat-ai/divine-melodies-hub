...
const SHANKH_ITEMS = [
  { id: 'conch', image: '/images/conch.png', nameHi: 'दक्षिणावर्ती शंख', nameEn: 'Dakshinavarti Shankh', category: 'popular' },
  { id: 'shankh-white', image: '/images/shankh.png', nameHi: 'श्वेत वामवर्ती शंख', nameEn: 'White Shankh', category: 'popular' },
]

export function ShankhPicker({
  open,
  onSelect,
  onClose,
}: {
  open: boolean
  onSelect: (shankhId: string, image: string) => void
  onClose: () => void
}) {
  const { lang } = useLang()
  const [selectedCategory, setSelectedCategory] = useState('popular')
  const [selectedShankhId, setSelectedShankhId] = useState('conch')

  const filteredItems = SHANKH_ITEMS.filter((item) => {
      ...
      {open && (
        ...
        >
          ...
          >
            ...
            {/* Header: Shankh Badge + Title & Subtitle + Close Button */}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-serif text-xl sm:text-2xl font-black text-[#3D1409] dark:text-[#FCEFD2] tracking-wide leading-tight">
                    {lang === 'hi' ? 'शंख नाद' : 'Blow Shankh'}
                  </h3>
                  <p className="font-serif text-xs text-[#6D4C3D] dark:text-[#fcefd2]/75 leading-snug">
                    {lang === 'hi' ? 'मंगलमय शंख ध्वनि का नाद करें' : 'Blow the auspicious conch shell'}
                  </p>
                </div>
              </div>
            ...
            {/* Shankh Cards Carousel */}
            <div className="no-scrollbar flex items-center gap-3 overflow-x-auto pt-1 pb-3 px-1">
              {filteredItems.map((item) => {
                const isSelected = selectedShankhId === item.id
                return (
                  <button
                    key={item.id}
                    type="button"
                    onClick={() => {
                      setSelectedShankhId(item.id)
