...
const DIYA_ITEMS = [
  { id: 'diya-brass', image: '/images/diya-brass.png', nameHi: 'पीतल का दीपक', nameEn: 'Brass Diya', category: 'popular' },
  { id: 'diya-classic', image: '/images/diya.png', nameHi: 'पंचमुखी दीपक', nameEn: 'Classic Diya', category: 'popular' },
]

export function DiyaPicker({
  open,
  onSelect,
  onClose,
}: {
  open: boolean
  onSelect: (diyaId: string, image: string) => void
  onClose: () => void
}) {
  const { lang } = useLang()
  const [selectedCategory, setSelectedCategory] = useState('popular')
  const [selectedDiyaId, setSelectedDiyaId] = useState('diya-brass')

  const filteredItems = DIYA_ITEMS.filter((item) => {
      ...
      {open && (
        ...
        >
          ...
          >
            ...
            {/* Header: Diya Badge + Title & Subtitle + Close Button */}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-serif text-xl sm:text-2xl font-black text-[#3D1409] dark:text-[#FCEFD2] tracking-wide leading-tight">
                    {lang === 'hi' ? 'दीपक प्रज्वलन' : 'Light Diya'}
                  </h3>
                  <p className="font-serif text-xs text-[#6D4C3D] dark:text-[#fcefd2]/75 leading-snug">
                    {lang === 'hi' ? 'अपने आराध्य के सम्मुख दीपक प्रज्वलित करें' : 'Light a sacred diya before your deity'}
                  </p>
                </div>
              </div>
            ...
            <div className="no-scrollbar flex items-center gap-3 overflow-x-auto pt-1 pb-3 px-1">
              {filteredItems.map((item) => {
                const isSelected = selectedDiyaId === item.id
                return (
                  <button
                    key={item.id}
                    type="button"
                    onClick={() => {
                      setSelectedDiyaId(item.id)
