  ...
  try {
    ...
    for (const file of files) {
      ...
      if (stat.isDirectory()) {
        ...
        if (stat.size > 50000 && stat.size < 250000) {
          try {
            // Read first 50,000 characters
            const fd = fs.openSync(fullPath, 'r');
            const buffer = Buffer.alloc(Math.min(stat.size, 50000));
            fs.readSync(fd, buffer, 0, buffer.length, 0);
            fs.closeSync(fd);
            const text = buffer.toString('utf8');
            if (text.includes('LIVE_WALLPAPERS_LIST') || text.includes('Vrindavan Raas Leela')) {
              console.log("Found in history file: " + fullPath);
